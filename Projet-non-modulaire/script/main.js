...
async function initModularUI() {
    ...
    if (launchSimulationBtn) {
        launchSimulationBtn.addEventListener('click', () => {
            ...
            setTimeout(() => {
                // FORCER le scénario choisi AVANT toute action
                window.currentScenario = AppState.scenario;

                // Afficher la page de simulation
                showMainContent();

                // Reconfigurer l’UI et les inputs
                updateSimulatorInputsDisplay();
                buildSyncedInputs('dashboardControls', window.currentScenario);
                buildSyncedInputs('fullscreenControls', window.currentScenario);

                // Lancer la simulation avec le bon scénario
                simulate();
                redrawAllCharts();
            }, 1500);
        });
...
------------------------------------------------- */
function simulate() {
    /* -- utilise TOUJOURS la valeur globale officielle -- */
    const activeScenario = window.currentScenario;

    console.log("LOG 5: Valeur de 'partis' au début de la fonction simulate :", partis);
    console.log("INFO X: Début de la simulation pour le scénario :", activeScenario);

    /* 1. Lecture unifiée des paramètres */
    const keys = activeScenario === 1
        ? ['pib_eu', 'gini_eu', 'co2_eu', 'veb_eu']
        : ['normandie_eu', 'vdem_eu', 'pib_growth_eu', 'idh_eu'];
    ...
    /* 2. Simulation EU */
    const euData = simulateEU(activeScenario, euInputsForSimulation);
    ...
    plotEU(simulationYears, euData, activeScenario);
    console.log('LOG ESSENTIEL', simulationYears, euData, activeScenario);
    ...
    /* 5. Reste inchangé */
    simulerPolitique(statsUE, activeScenario);
    updatePolitiqueUI();
    simulerLobbies(statsUE, activeScenario);
    createLobbyChart();
    updateCountryChartForSelectedCountry();
    console.log('simulate() called with scenario :', activeScenario);
...
function updateSimulatorInputsDisplay() {
    /* -- utilise TOUJOURS la valeur globale -- */
    const activeScenario = window.currentScenario;
