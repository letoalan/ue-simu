import { AppState, simulationYears, partis, lobbies, pondPartis, SimulationCache } from '../../../../state/index.js';
...
export function runFullSimulation() {
    ...
    // 2. Création d'une clé de cache unique basée sur les entrées
    const sortedKeys = Object.keys(euInputs).sort();
    const cacheKey = `fullsim-${activeScenario}-${sortedKeys.map(k => `${k}:${euInputs[k]}`).join('|')}`;

    // 3. Vérifier si les résultats sont déjà en cache
    if (SimulationCache.has(cacheKey)) {
        console.log("Moteur de simulation: Démarrage pour le scénario:", activeScenario, "(depuis le cache)");
        const cachedResult = SimulationCache.get(cacheKey);

        // Restaurer l'état des partis et lobbies depuis le cache pour assurer la cohérence de l'UI
        Object.keys(cachedResult.partisState).forEach(pKey => {
            if (partis[pKey]) Object.assign(partis[pKey], cachedResult.partisState[pKey]);
        });
        Object.keys(cachedResult.lobbiesState).forEach(lKey => {
            if (lobbies[lKey]) Object.assign(lobbies[lKey], cachedResult.lobbiesState[lKey]);
        });

        return { euData: cachedResult.euData, localStatsUE: cachedResult.localStatsUE };
    }

    console.log("Moteur de simulation: Démarrage pour le scénario:", activeScenario, "(nouveau calcul)");

    // 4. Simulation UE (utilise son propre cache interne, bénéfique pour les autres modules)
    ...
    // 7. Mettre en cache les résultats complets avant de retourner
    const resultsToCache = {
        euData,
        localStatsUE,
        partisState: JSON.parse(JSON.stringify(partis)), // Copie profonde pour l'état
        lobbiesState: JSON.parse(JSON.stringify(lobbies))
    };
    SimulationCache.set(cacheKey, resultsToCache);

    // 8. Retourner les données pour la mise à jour de l'UI
...
export function simulateEU(scenario, inputValues) {
    // Création d'une clé de cache robuste et ordonnée
    const sortedKeys = Object.keys(inputValues).sort();
    const cacheKey = `eu-${scenario}-${sortedKeys.map(k => `${k}:${inputValues[k]}`).join('|')}`;

    // Vérifier si le résultat est déjà en cache
    if (SimulationCache.has(cacheKey)) {
        console.log("Simulation UE - Scénario:", scenario, "(depuis le cache)");
        return SimulationCache.get(cacheKey);
    }

    console.log("Simulation UE - Scénario:", scenario, "(nouveau calcul)");
    ...
    // Mettre en cache le nouveau résultat avant de le retourner
    SimulationCache.set(cacheKey, euData);
...
export function simulateCountry(profile, euInputs, scenario) {
    ...
    const sortedKeys = Object.keys(euInputs).sort();
    const cacheKey = `country-${profile.name}-${scenario}-${sortedKeys.map(k => `${k}:${euInputs[k]}`).join('|')}`;

    // Vérification du cache
    if (SimulationCache.has(cacheKey)) {
        console.log("Simulation pays:", profile.name, "- Scénario:", scenario, "(depuis le cache)");
        return SimulationCache.get(cacheKey);
    }

    console.log("Simulation pays:", profile.name, "- Scénario:", scenario, "(nouveau calcul)");
